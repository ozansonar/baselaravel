@extends('layouts.auth')

@section('title', 'Şifremi Unuttum')

@section('content')
    <h1 class="auth-card__title">Şifrenizi mi unuttunuz?</h1>
    <p class="auth-card__sub">E-postanıza sıfırlama bağlantısı gönderelim</p>

    <form method="POST" action="{{ route('password.email') }}">
        @csrf

        {{-- Email --}}
        <div class="mb-3">
            <label for="email" class="form-label">E-posta Adresi</label>
            <div class="input-group">
                <span class="input-group-text"><i class="fa-solid fa-envelope"></i></span>
                <input type="email"
                       class="form-control @error('email') is-invalid @enderror"
                       id="email" name="email" value="{{ old('email') }}"
                       placeholder="ornek@mail.com" required autofocus autocomplete="email">
                @error('email')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        </div>

        <x-recaptcha />

        <button type="submit" class="btn btn-primary w-100">
            <i class="fa-solid fa-paper-plane"></i> Bağlantı Gönder
        </button>
    </form>

    <p class="text-center text-muted mt-4 mb-0">
        <a href="{{ route('login') }}" class="fw-semibold">
            <i class="fa-solid fa-arrow-left"></i> Giriş sayfasına dön
        </a>
    </p>
@endsection
