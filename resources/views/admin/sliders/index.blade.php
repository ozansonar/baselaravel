...
@section('content')
        ...
        <div class="card-body-custom">
            <form method="GET" action="{{ route('admin.sliders.index') }}" class="cl-toolbar" id="filterForm">
                ...
                <div class="cl-toolbar-actions">
                    ...
                    <x-export-menu export="sliders" :total="$sliders->total()" />
                    ...
                    <div class="cl-bulk-actions d-none" data-bulk-bar>
                        ...
                        @if(request('status') === 'trashed')
                            <button type="button" class="usr-action-btn success"
                                    ...
                                    data-bulk-message=":count slider geri yüklenecek. Onaylıyor musunuz?"
                        ...
                        @else
                            <button type="button" class="usr-action-btn danger"
                                    ...
                                    data-bulk-message=":count slider silinecek. Silinenler &quot;Silinmiş&quot; sekmesinden geri alınabilir."
                    ...
                    <tbody id="slidersTableBody">
                        @forelse($sliders as $slider)
                            <tr data-status="{{ $slider->trashed() ? 'trashed' : ($slider->is_active ? 'active' : 'passive') }}">
                                <td data-label="Seç"><input type="checkbox" class="usr-checkbox" data-bulk-item value="{{ $slider->id }}" data-fv-ignore></td>
    ...
    <form method="POST" action="{{ route('admin.sliders.bulk-destroy', request()->query()) }}" id="bulkDeleteForm" class="d-none">
        @csrf
        @method('DELETE')
    </form>
    <form method="POST" action="{{ route('admin.sliders.bulk-restore', request()->query()) }}" id="bulkRestoreForm" class="d-none">
        @csrf
        @method('PATCH')
    </form>
@endsection
