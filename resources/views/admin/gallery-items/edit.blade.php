...
@section('title', 'Galeri Öğesi Düzenle')
...
@section('content')
    ...
    <div class="page-header d-flex align-items-start align-items-sm-center justify-content-between flex-column flex-sm-row gap-3 mb-4" data-aos="fade-down">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="page-title mb-0">Galeri Öğesi Düzenle</h1>
                <p class="page-subtitle mb-0">{{ $item->title }}</p>
            </div>
        </div>
    </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                          ...
                                          placeholder="Kısa bir açıklama girin...">{{ old('description', $item->description) }}</textarea>
                                @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('gallery_category_id') is-invalid @enderror"
                                    ...
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ (int) old('gallery_category_id', $item->gallery_category_id) === $cat->id ? 'selected' : '' }}>
                                            {{ $cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row g-3">
                            @if($item->image)
                                <div class="col-12">
                                    ...
                                    <div class="mb-3">
                                        <img src="{{ upload_url($item->image, 'md') }}" alt="{{ $item->title }}"
                            ...
                            <div class="col-12">
                                <label class="form-label" for="image">
                                    {{ $item->image ? 'Yeni Görsel (opsiyonel)' : 'Görsel' }}
                                </label>
                                <input type="file"
                                ...
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">PNG, JPG, WebP | Maks: 4 MB | Boş bırakılırsa mevcut görsel korunur</div>
                            </div>
                        </div>
                    </div>
                </div>
