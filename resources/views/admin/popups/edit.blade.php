...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.popups.update', $popup) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                          ...
                                          placeholder="Popup açıklamasını girin...">{{ old('description', $popup->description) }}</textarea>
                                @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row g-3">
                            @if($popup->image)
                                <div class="col-12">
                                    ...
                                    <div>
                                        <img src="{{ upload_url($popup->image, 'md') }}"
                                             alt="{{ $popup->title }}"
                            ...
                            <div class="col-12">
                                <label class="form-label" for="image">
                                    {{ $popup->image ? 'Görseli Değiştir' : 'Popup Görseli' }}
                                </label>
                                <input type="file"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('size') is-invalid @enderror"
                                    ...
                                    @foreach(\App\Enums\PopupSize::cases() as $size)
                                        <option value="{{ $size->value }}" {{ old('size', $popup->size->value) === $size->value ? 'selected' : '' }}>
                                            {{ $size->label() }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number"
                                       ...
                                       value="{{ old('sort_order', $popup->sort_order) }}" min="0">
                                @error('sort_order')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select @error('is_active') is-invalid @enderror"
                                    ...
                                    <option value="1" {{ old('is_active', $popup->is_active) == 1 ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('is_active', $popup->is_active) == 0 ? 'selected' : '' }}>Pasif</option>
                                </select>
                                @error('is_active')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                        ...
                                        <div class="col-6 col-md-4 col-lg-3">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox"
                                                       ...
                                                       id="page_{{ $page->value }}"
                                                       {{ in_array($page->value, old('pages', $popup->pages ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('start_date', $popup->start_date?->format('Y-m-d')) }}">
                                @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('end_date', $popup->end_date?->format('Y-m-d')) }}">
                                @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
