...
@section('page_title', 'Değerlendirmeler')
@section('page_description', 'Ürün değerlendirmelerini yönetin')
...
@section('content')
    ...
    <nav aria-label="breadcrumb" class="mb-3" data-aos="fade-down" data-aos-duration="400">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active text-teal">Değerlendirmeler</li>
        </ol>
    </nav>
    ...
    <div class="page-header d-flex align-items-center justify-content-between flex-wrap gap-3" data-aos="fade-down">
        <div>
            ...
            <p class="page-subtitle">
                @if($pendingCount > 0)
                    {{ $pendingCount }} onay bekleyen değerlendirme var
                @else
                    Tüm ürün değerlendirmelerini yönetin
                @endif
            </p>
        </div>
        <a href="{{ route('admin.ai-reviews.create') }}" class="btn-teal">
            <i class="bi bi-robot me-1"></i> AI Yorum Üret
        </a>
    </div>
    ...
    <div class="cl-status-tabs mb-4" data-aos="fade-up" data-aos-delay="100">
        ...
        <a href="{{ route('admin.reviews.index', array_merge(request()->except(['status', 'page']), ['status' => 'rejected'])) }}"
        ...
        <a href="{{ route('admin.reviews.index', array_merge(request()->except(['status', 'page']), ['status' => 'trashed'])) }}"
    ...
    <div class="card-dark mb-4" data-aos="fade-up" data-aos-delay="150">
        <div class="card-body-custom">
            <form method="GET" action="{{ route('admin.reviews.index') }}" class="cl-toolbar" id="filterForm">
                @if(request('status'))
                    <input type="hidden" name="status" value="{{ request('status') }}">
                @endif
                <div class="cl-search">
                    <i class="bi bi-search"></i>
                    <input type="text" name="search" id="reviewSearch" placeholder="Ad, e-posta veya yorum ile ara..." value="{{ request('search') }}">
                </div>
                <div class="cl-toolbar-actions">
                    <a href="{{ route('admin.reviews.index') }}" class="cl-filter-reset" title="Filtreleri Sıfırla">
    ...
    <div class="card-dark mb-4" data-aos="fade-up" data-aos-delay="200">
                ...
                <table class="cl-table">
                    <thead>
                        <tr>
                            ...
                            <th>Puan</th>
                            <th>Kişi</th>
                            <th class="d-none d-md-table-cell">Ürün</th>
                            <th class="d-none d-lg-table-cell">Yorum</th>
                            <th>Durum</th>
                    ...
                    <tbody id="reviewsTableBody">
                        @forelse($reviews as $review)
                            <tr data-status="{{ $review->trashed() ? 'trashed' : $review->status->value }}">
                                <td data-label="Seç"><input type="checkbox" class="usr-checkbox review-checkbox" value="{{ $review->id }}" onchange="updateBulk()"></td>
                                <td data-label="Puan">
                                ...
                                <td data-label="Kişi">
                                    <div class="cl-content-info">
                                        <span class="cl-content-title">{{ $review->name }}</span>
                                        <span class="cl-content-meta"><i class="bi bi-envelope me-1"></i>{{ $review->email }}</span>
                                    </div>
                                </td>
                                <td data-label="Ürün" class="d-none d-md-table-cell">
                                    @if($review->product)
                                        <span class="cl-content-meta text-teal">{{ Str::limit($review->product->name, 30) }}</span>
                                    @else
                                        <span class="text-muted">—</span>
                                    @endif
                                </td>
                                <td data-label="Yorum" class="d-none d-lg-table-cell">
                                    <span class="cl-content-meta">{{ Str::limit($review->comment, 50) }}</span>
                                </td>
                                <td data-label="Durum">
                                    @if($review->trashed())
                                ...
                                <td data-label="Tarih" class="d-none d-lg-table-cell">
                                    <div class="cl-content-info">
                                        <span class="usr-meta">{{ $review->created_at->format('d.m.Y') }}</span>
                                        <span class="cl-content-meta">{{ $review->created_at->format('H:i') }}</span>
                                    </div>
                                </td>
                                <td data-label="İşlem">
                                    <div class="usr-actions">
                                        @if($review->trashed())
                                            <form method="POST" action="{{ route('admin.reviews.restore', $review->id) }}" class="d-inline" id="restoreForm-{{ $review->id }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="button" class="usr-action-btn success" title="Geri Yükle" onclick="confirmReviewAction('restore', {{ $review->id }}, '{{ addslashes($review->name) }}')"><i class="bi bi-arrow-counterclockwise"></i></button>
                                            </form>
                                        @else
                                            <a href="{{ route('admin.reviews.show', $review) }}" class="usr-action-btn" title="Detay"><i class="bi bi-eye"></i></a>
                                            @if($review->status !== \App\Enums\ReviewStatus::Approved)
                                                <form method="POST" action="{{ route('admin.reviews.approve', $review) }}" class="d-inline" id="approveForm-{{ $review->id }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="button" class="usr-action-btn success" title="Onayla" onclick="confirmReviewAction('approve', {{ $review->id }}, '{{ addslashes($review->name) }}')"><i class="bi bi-check-lg"></i></button>
                                                </form>
                                            @endif
                                            @if($review->status !== \App\Enums\ReviewStatus::Rejected)
                                                <form method="POST" action="{{ route('admin.reviews.reject', $review) }}" class="d-inline" id="rejectForm-{{ $review->id }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="button" class="usr-action-btn warning" title="Reddet" onclick="confirmReviewAction('reject', {{ $review->id }}, '{{ addslashes($review->name) }}')"><i class="bi bi-x-lg"></i></button>
                                                </form>
                                            @endif
                                            <button class="usr-action-btn danger" title="Sil" onclick="openDeleteModal({{ $review->id }}, '{{ addslashes($review->name) }}')"><i class="bi bi-trash"></i></button>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-5">
                                    <i class="bi bi-chat-square-text d-block fs-1 mb-2"></i>
        ...
        @include('partials.admin.pagination', ['paginator' => $reviews, 'itemLabel' => 'değerlendirme'])
    </div>

@endsection

@push('scripts')
<script src="{{ versioned_asset('assets/admin/js/reviews.js') }}"></script>
@endpush
