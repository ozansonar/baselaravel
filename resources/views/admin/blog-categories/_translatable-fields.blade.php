    ...
    <div class="card-dark mb-4" id="section-basic_{{ $language->code }}">
        ...
        <div class="card-body-custom">
            <div class="row g-3">
                ...
                <!-- İkon -->
                <div class="col-12">
                    <label class="form-label" for="icon_{{ $language->code }}">Bootstrap Icon Sınıfı</label>
                    <div class="input-group">
                        <span class="input-group-text" id="iconPreview">
                            <i class="bi {{ old("translations.{$language->code}.icon", $isEdit ? $category->icon : '') ?: 'bi-tag' }}"></i>
                        </span>
                        <input
                            type="text"
                            class="form-control @error("translations.{$language->code}.icon") is-invalid @enderror"
                            id="icon_{{ $language->code }}"
                            name="translations[{{ $language->code }}][icon]"
                            value="{{ old("translations.{$language->code}.icon", $isEdit ? $category->icon : '') }}"
                            placeholder="bi-heart-fill"
                            oninput="updateIconPreview(this.value)"
                        >
                    </div>
                    @error("translations.{$language->code}.icon")
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">Bootstrap Icons sınıf adı (ör: bi-heart-fill, bi-book, bi-star)</div>
                </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input
                        ...
                        name="translations[{{ $language->code }}][sort_order]"
