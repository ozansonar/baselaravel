...
@section('title', 'Adresi Düzenle | ' . \App\Models\Setting::getValue('site_name', config('app.name')))
@section('meta_description', 'Teslimat adresinizi düzenleyin.')
            ...
            <div class="col-lg-9">
                <div class="dashboard-content">
                    <div class="content-header">
                        <h3><i class="fa-solid fa-pen-to-square"></i> Adresi Düzenle</h3>
                        <a href="{{ route('account.addresses') }}" class="btn-view">
                            <i class="fa-solid fa-arrow-left me-1"></i> Geri
                        </a>
                    </div>

                    <form method="POST" action="{{ route('account.addresses.update', $address) }}" class="profile-form">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('full_name') is-invalid @enderror"
                                           id="full_name" name="full_name" value="{{ old('full_name', $address->full_name) }}" required>
                                    @error('full_name')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('zip_code') is-invalid @enderror"
                                   id="zip_code" name="zip_code" value="{{ old('zip_code', $address->zip_code) }}">
                            @error('zip_code')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('address_line') is-invalid @enderror"
                                      ...
                                      placeholder="Mahalle, sokak, bina no, daire no" required>{{ old('address_line', $address->address_line) }}</textarea>
                            @error('address_line')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
