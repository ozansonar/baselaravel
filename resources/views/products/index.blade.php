...
@section('title', $category->meta_title ?? $category->name . ' | ' . \App\Models\Setting::getValue('site_name', config('app.name')))
@section('meta_description', $category->meta_description ?? $category->description)
@section('canonical', request()->fullUrlWithQuery(request()->query('page') ? ['page' => request()->query('page')] : []))
@if($category->image)
@section('og_image', url(upload_url($category->image)))
@endif
...
@php
    $catBreadcrumbJsonLd = [
        ...
        'itemListElement' => [
            ...
            ['@type' => 'ListItem', 'position' => 2, 'name' => 'Ürünlerimiz', 'item' => route('products.all')],
            ['@type' => 'ListItem', 'position' => 3, 'name' => $category->name],
        ],
    ];
@endphp
<script type="application/ld+json">{!! json_encode($catBreadcrumbJsonLd, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}</script>
@endpush
    ...
    <div class="container">
        <div class="page-header-content">
            <nav class="breadcrumb-custom animate-fade-up" aria-label="Breadcrumb">
                ...
                <a href="{{ route('products.all') }}">Ürünlerimiz</a>
                <i class="fas fa-chevron-right"></i>
                <span>{{ $category->name }}</span>
            </nav>
            <h1 class="page-title animate-fade-up delay-1">{{ $category->name }}</h1>
...
<section class="filter-section" aria-labelledby="filter-heading">
    <div class="container">
        <h2 class="visually-hidden" id="filter-heading">Filtreler</h2>
...
<section class="products-section" aria-labelledby="products-heading">
    <div class="container">
        <h2 class="visually-hidden" id="products-heading">{{ $category->name }} Ürünleri</h2>
        ...
        <div class="text-center py-5">
            ...
            <h3 class="text-brown-light">Bu kategoride henüz ürün bulunmuyor.</h3>
            <a href="{{ route('products.all') }}" class="btn-custom mt-3">
