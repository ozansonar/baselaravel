...
(function ($) {
    ...
    if ($.validationEngineLanguage && $.validationEngineLanguage.allRules) {
        ...
        // validate[custom[dateTime]] — <input type="datetime-local"> değeri.
        // Paketten gelen custom[date] yalnız YYYY-MM-DD kabul ediyor, oysa bu
        // girdi tarayıcıdan hep "2026-08-28T20:01" biçiminde geliyor; kural
        // dolu her alanı reddediyordu. Saniye isteğe bağlı: step ayarına göre
        // bazı tarayıcılar ":ss" de ekliyor.
        allRules.dateTime = {
            regex: /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/,
            alertText: 'Geçerli bir tarih ve saat seçin'
        };
