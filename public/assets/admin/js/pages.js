(function () {
  ...
  window.toggleSelectAll = function (master) {
    document.querySelectorAll('#pagesTableBody .page-checkbox').forEach(function (cb) {
  ...
  window.updateBulk = function () {
    var checked = document.querySelectorAll('#pagesTableBody .page-checkbox:checked').length;
    ...
    var total = document.querySelectorAll('#pagesTableBody .page-checkbox').length;
    all.indeterminate = checked > 0 && checked < total;
    all.checked       = checked === total && total > 0;
  };

  /* ==================== DELETE MODAL ==================== */
  window.openDeleteModal = function (pageId, pageName) {
    AdminModal.confirm({
      title: 'Silme Onayı',
  ...
  window.openBulkDeleteModal = function () {
    var checked = document.querySelectorAll('#pagesTableBody .page-checkbox:checked');
    if (checked.length === 0) {
      showToast('Lütfen sayfa seçin', 'warning');
      return;
    }

    AdminModal.confirm({
      title: 'Toplu Silme Onayı',
      message: checked.length + ' sayfayı silmek istediğinize emin misiniz?',
      type: 'danger',
      confirmText: 'Evet, Sil',
      confirmIcon: 'bi bi-trash3',
      warning: 'Bu işlem geri alınamaz.'
      ...
      ids.forEach(function (id) {
        fetch('/admin/pages/' + id, {
          ...
          if (completed === ids.length) {
            showToast(ids.length + ' sayfa silindi', 'success');
