(function () {
  ...
  window.toggleSelectAll = function (master) {
    document.querySelectorAll('#categoriesTableBody .category-checkbox').forEach(function (cb) {
  ...
  window.updateBulk = function () {
    var checked = document.querySelectorAll('#categoriesTableBody .category-checkbox:checked').length;
    ...
    var total = document.querySelectorAll('#categoriesTableBody .category-checkbox').length;
  ...
  window.openDeleteModal = function (categoryId, categoryName) {
    AdminModal.confirm({
      ...
      warning: 'Kategori silindiğinde içerikler kategorisiz kalır.'
  ...
  window.openBulkDeleteModal = function () {
    var checked = document.querySelectorAll('#categoriesTableBody .category-checkbox:checked');
    if (checked.length === 0) {
    ...
    AdminModal.confirm({
      ...
      message: checked.length + ' kategoriyi silmek istediğinize emin misiniz?',
      type: 'danger',
      confirmText: 'Evet, Sil',
      confirmIcon: 'bi bi-trash3',
      ...
      ids.forEach(function (id) {
        fetch('/admin/blog-categories/' + id, {
          ...
          if (completed === ids.length) {
            showToast(ids.length + ' kategori silindi', 'success');
