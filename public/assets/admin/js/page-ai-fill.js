/**
 * Page AI Fill
 * Generate page content via AI and populate all form fields.
 */
(function () {
    'use strict';

    window.aiFillPageAll = function () {
        var btn = document.getElementById('btnAiFillPage');
        if (!btn) return;
        ...
        if (title === '') {
            if (typeof AdminModal !== 'undefined') {
                AdminModal.status({
                    title: 'Eksik Bilgi',
                    message: 'Lütfen önce sayfa başlığını girin.',
                    type: 'warning'
                });
            }
            titleInput.focus();
        ...
        if (typeof AdminModal !== 'undefined') {
            AdminModal.loading.show({
                title: 'AI Üretiyor...',
                message: 'Sayfa içeriği oluşturuluyor, lütfen bekleyin.'
            });
        }
        ...
        fetch(url, {
            ...
            .then(function (res) {
                ...
                if (typeof AdminModal !== 'undefined') {
                    AdminModal.status({
                        title: 'Başarılı',
                        message: 'Sayfa içeriği AI ile dolduruldu. Göndermeden önce kontrol edin.',
                        type: 'success'
                    });
                }
            })
